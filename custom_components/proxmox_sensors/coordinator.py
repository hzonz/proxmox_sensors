...
async def create_proxmox_coordinator(hass, entry, client):
    ...
    # Extraemos las nuevas opciones de filtrado
        ...
        try:
            async with asyncio.timeout(30):
                ...
                if server_type == "PBS":
                    ...
                    actual_stores = data.get("selected_storage") or await client.get_pbs_datastores(hass)

                    for store in actual_stores or []:
                        status = await client.get_pbs_datastore_status(hass, store)
                        usage = await client.get_pbs_datastore_usage(hass, store)
                    ...
                    result["pbs_datastores"] = datastores_data
                    ...
                    if enable_pbs_tasks:
                        ...
                        result["pbs_tasks"] = tasks[0] if tasks else {}
